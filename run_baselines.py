...
def build_environment(
    seed: int,
    baseline: str,
    scenario_name: str = "toy",
    physical_nodes: int = 6,
    virtual_nodes: int = 3,
) -> DynamicTopologyEnv:
    scenario = DynamicSAGINScenario(ScenarioConfig(scenario_name, physical_nodes, virtual_nodes), seed)
    history = scenario.initial_history()
...
def main() -> None:
    ...
    for seed in range(args.seeds):
        for baseline in baseline_names:
            environment = build_environment(seed, baseline, args.scenario, args.physical_nodes, args.virtual_nodes)
            result = environment.run(args.steps)
            rows.append({"seed": seed, "scenario": args.scenario, "baseline": baseline, **result["metrics"]})

    metric_names = [name for name in rows[0] if name not in {"seed", "scenario", "baseline"}]
    ...
    write_csv(args.output / "baseline_runs.csv", rows, ["seed", "scenario", "baseline", *metric_names])
