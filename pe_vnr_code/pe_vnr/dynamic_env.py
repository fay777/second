...
class DynamicTopologyEnv:
    def __init__(
        ...
        self.planning_env = planning_env or PlanningEnv(planner.config.max_trigger_delay)
    ...
    def _planning_observation(self, prediction) -> PlanningObservation:
        ...
        deployed_node_risk = [prediction.node_risk.get(node_id, 0.0) for node_id in self.service.deployment.node_mapping.values()]
        deployed_link_risk = [
            prediction.link_risk.get(edge_key(*link_id), 0.0)
            for path in self.service.deployment.link_mapping.values()
            for link_id in path
        ]
        future_node_mean = [
            np.mean(list(risk.values())) if risk else 0.0 for risk in prediction.future_node_risk
        ]
        future_link_mean = [
            np.mean(list(risk.values())) if risk else 0.0 for risk in prediction.future_link_risk
        ]
        ...
        risk_features = np.array(
            [
                ...
                np.mean(future_node_mean) if future_node_mean else 0.0,
                np.mean(future_link_mean) if future_link_mean else 0.0,
            ],
            dtype=np.float32,
        )
        ...
        return self.planning_env.build_state(service_features, risk_features, cost_features)
    ...
    def _finish_training_decision(
        ...
        reward = 1.0 - post_risk - 0.6 * realized_cost - 0.4 * disruption
        if sla_violated:
            reward -= 1.5
        if failure:
            reward -= 1.0
        ...
        if self.service.remaining_lifetime == 0:
            ...
            return MDPTransition(None, reward, True, "terminated", {"plan": plan, "failure": failure})
        self.step_time()
        self._training_prediction = self.predictor.predict(self.topology)
        self._training_stage = "planning"
        return MDPTransition(
            ...
            False,
            "planning",
            {"plan": plan, "migrated": migrated, "post_risk": post_risk, "sla_violated": sla_violated},
        )
    ...
    def planning_step(self, action: int) -> MDPTransition:
        ...
        try:
            observation = self.execution_env.reset(self.topology.current, self.service, plan, prediction.node_risk)
        except RuntimeError:
            return self._finish_training_decision(plan, service_before, prediction, False, failure=True)
    ...
    def _service_post_risk(self, deployment, prediction) -> float:
        node_scores = [
            prediction.node_risk.get(p_node_id, 0.0)
            for p_node_id in deployment.node_mapping.values()
        ]
        link_scores = []
        for path in deployment.link_mapping.values():
            for link_id in path:
                link_scores.append(prediction.link_risk.get(edge_key(*link_id), 0.0))
